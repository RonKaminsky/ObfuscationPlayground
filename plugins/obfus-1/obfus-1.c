...
int forward(char *inputString, int inputLengthInBytes, char *outputString)
{
   ...
   {
      outputString[k] = inputString[k] ^ byte_perm[outputString[k - 1]];
   }

   outputString[inputLengthInBytes - 1] ^= 131;
   
   for (int k = inputLengthInBytes - 2; k >= 0; --k)
   {
      outputString[k] = outputString[k] ^ byte_perm[outputString[k + 1]];
   }
...
int backward(char *inputString, int inputLengthInBytes, char *outputString)
{
   ...
   {
      outputString[k] = inputString[k] ^ byte_perm[inputString[k + 1]];
   }

   outputString[inputLengthInBytes - 1] = inputString[inputLengthInBytes - 1] ^ 131;
